...
#'
#' @description This function visualizes followup status by site for Clinical followup form(crf12) and patient
...
df_expected_2wk <- df %>% 
  ...
  group_by(facilitycode) %>% 
  summarise("Eligible & Enrolled"= sum(eligible_enrolled, na.rm = TRUE), "DF complete"= sum(dwc_complete, na.rm = TRUE),
            "2 Weeks"= sum(followup_expected_2wk, na.rm = TRUE)) %>% 
  rename("Expected" = `2 Weeks`) %>% 
  left_join(df_crf12) %>% 
