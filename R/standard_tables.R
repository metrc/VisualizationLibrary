...
#' 
survival_analysis_kaplan_meier <- function(analytic, type_construct, days_construct, outcome_length, pre_filter_constructs=NULL, remove_zero_day_events=TRUE, arm_labels = c(`0` = "Control", `1` = "Treatment"), outcome_label="Outcome"){
  # ── Prep data ───────────────────────────────────────────────────────────
  df <- analytic %>%
    filter(enrolled == 1) %>%
