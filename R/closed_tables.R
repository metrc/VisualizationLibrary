...
#' @param include_ae boolean to include adverse events in the table, defaults to FALSE
...
closed_not_complete_sae_deviation_by_type <- function(analytic, include_ae=FALSE, factors=list()){
  ...
  analytic <- if_needed_generate_example_data(
    ...
    example_constructs = c("enrolled", "not_expected_reason", "not_completed_reason",
                           ...
                           "not_completed", "consented", "ae_count"), 
    example_types = c("Boolean", "Category", "Category-U3", "Category-U2", "Category-U3",
                      "Category-U3", "Number-U5", "TreatmentArm", "Boolean", "Boolean", "Boolean"))
  ...
  inner_closed_not_complete_sae_deviation_by_type <- function(inner_analytic, group){
    ...
    not_completed_df <- inner_analytic %>% 
      ...
      mutate(cat_order=1)
    not_completed_df_tot <- tibble(type = "Not Completed", n = sum(not_completed_df$n), cat_order=2)
    ...
    consented_df <- tibble(type = " ", n = paste0("n=", consented, ' <sub>(Consented)</sub>'), cat_order=9)
    
    # --- Deviations (Screen & Consent)
    deviation_sc_df <- inner_analytic %>% 
    ...
    deviation_sc_tot <- tibble::tibble(type = "Screen and Consent",     n = sum(deviation_sc_df$n), cat_order=16)
    deviation_p_tot  <- tibble::tibble(type = "Procedural",             n = sum(deviation_p_df$n), cat_order=17)
    deviation_a_tot  <- tibble::tibble(type = "Administrative/Other",   n = sum(deviation_a_df$n), cat_order=18)
    deviation_df_tot <- tibble::tibble(type = "Protocol Deviations",    n = sum(deviation_sc_df$n) + sum(deviation_p_df$n) + sum(deviation_a_df$n), cat_order=19)
  ...
  second_indent_idx <- which(df_final$cat_order > 9 & df_final$cat_order <19)
...
closed_not_complete_sae_deviation_by_type_auto_categories <- function(analytic, category_defaults=c("Safety","Informed Consent","Eligibility","Protocol Implementation","Other"), include_ae=FALSE){
  ...
  analytic <- if_needed_generate_example_data(
    analytic, 
    example_constructs = c('treatment_arm', 'enrolled', "protocol_deviation_full_data", "not_expected_reason", 'not_completed', 
                           'not_completed_reason', 'sae_count', 'consented', "ae_count"), 
    example_types = c('TreatmentArm','Boolean', "(';new_row: ', '|')FacilityCode|Date|Category|Date|Category|Character", 'Category', 'Boolean',
  ...
  inner_closed_not_complete_sae_deviation_by_type <- function(inner_analytic, group){
    ...
    if (include_ae) {
      df_final_top <- bind_rows(
        not_completed_df_tot %>% mutate(level = 0L) %>% mutate(n=as.character(n)) %>% mutate(cat_order=1),
        not_completed_df     %>% mutate(level = 1L) %>% mutate(n=as.character(n)) %>% mutate(cat_order=2),
        not_expected_df_tot  %>% mutate(level = 0L) %>% mutate(n=as.character(n)) %>% mutate(cat_order=3),
        not_expected_df_tot  %>% mutate(level = 0L) %>% mutate(n=as.character(n)) %>% mutate(cat_order=4),
        sae_df               %>% mutate(level = 0L) %>% mutate(n=as.character(n)) %>% mutate(cat_order=5),
        ae_df                %>% mutate(level = 0L) %>% mutate(n=as.character(n)) %>% mutate(cat_order=6),
        consented_df         %>% mutate(level = 0L) %>% mutate(cat_order=7)
      )
    } else {
      df_final_top <- bind_rows(
        not_completed_df_tot %>% mutate(level = 0L) %>% mutate(n=as.character(n)) %>% mutate(cat_order=1),
        not_completed_df     %>% mutate(level = 1L) %>% mutate(n=as.character(n)) %>% mutate(cat_order=2),
        not_expected_df_tot  %>% mutate(level = 0L) %>% mutate(n=as.character(n)) %>% mutate(cat_order=3),
        not_expected_df      %>% mutate(level = 1L) %>% mutate(n=as.character(n)) %>% mutate(cat_order=4),
        sae_df               %>% mutate(level = 0L) %>% mutate(n=as.character(n)) %>% mutate(cat_order=5),
        consented_df         %>% mutate(level = 0L) %>% mutate(cat_order=6)
      )
    }
    
    # Bottom section (deviations): category headers level 0, their types level 1
    df_final_bottom <- deviation_df_tot %>% mutate(level = 0L) %>% mutate(cat_order=8)
    start_cat <- 1
    for (category_i in category_defaults) {
      category_df <- deviations_df %>%
        filter(category == category_i) %>%
        select(-category) %>%
        mutate(cat_order=10+start_cat)
  ...
  df_table <- table_tot %>%
    ...
    left_join(table_b) %>%
    arrange(o) %>%
    select(type, n_a, n_b, n_total, level) %>%
    mutate(across(c(n_a, n_b, n_total), ~ replace_na(.x, "0 (0%)"))) %>%
    mutate(type = if_else(str_detect(type, "^Other"), "Other", type))
  
  # Indent only child rows (types) using the level flag from Total
  indents_vec <- which(df_table$level == 1L)
  if(include_ae){
...
#' 
closed_enrollment_by_site_last_days_var_disc <- function(analytic, days = 0, 
  ...
  if(is.null(footnotes)){
    out <- paste0("<h4> </h4><br /><h4>Group A</h4><br />",
                  enrollment_by_site_last_days_var_disc(df_a, days, discontinued, discontinued_colname, include_exclusive_safety_set, average, cumulative_data),
                  "<h4>Group B</h4><br />",
                  enrollment_by_site_last_days_var_disc(df_b, days, discontinued, discontinued_colname, include_exclusive_safety_set, average, cumulative_data))
  } else{
    out <- paste0("<h4> </h4><br /><h4>Group A</h4><br />",
                  enrollment_by_site_last_days_var_disc(df_a, days, discontinued, discontinued_colname, include_exclusive_safety_set, average, cumulative_data) %>% add_footnote(footnotes, notation="number", escape = FALSE),
                  "<h4>Group B</h4><br />",
                  enrollment_by_site_last_days_var_disc(df_b, days, discontinued, discontinued_colname, include_exclusive_safety_set, average, cumulative_data) %>% add_footnote(footnotes, notation="number", escape = FALSE))
  }
...
#'
#' @description 
#' This is the closed version of the enrollment_by_site_last_days_var_disc_i function; see its documentation
#' for details.
#'
#' @param analytic analytic data set that must include constructs screened, eligible, refused, consented, 
#' not_consented, not_randomized, consented_and_randomized, enrolled, site_certification_date, 
...
#'
#' @examples
#' closed_enrollment_by_site_last_days_var_disc_i("Replace with Analytic Tibble")
#' closed_enrollment_by_site_last_days_var_disc_i("Replace with Analytic Tibble", days = 100)
#' closed_enrollment_by_site_last_days_var_disc_i("Replace with Analytic Tibble", include_exclusive_safety_set = FALSE, footnotes = TRUE)
#' closed_enrollment_by_site_last_days_var_disc_i("Replace with Analytic Tibble", include_exclusive_safety_set = TRUE)
#' 
closed_enrollment_by_site_last_days_var_disc_i <- function(analytic, days = 0, 
                                                           average = FALSE, 
                                                           cumulative_data = TRUE, 
                                                           footnotes=NULL){
  ...
  if(is.null(footnotes)){
    out <- paste0("<h4> </h4><br /><h4>Group A</h4><br />",
                  enrollment_by_site_last_days_var_disc_i(df_a, days, average, cumulative_data),
                  "<h4>Group B</h4><br />",
                  enrollment_by_site_last_days_var_disc_i(df_b, days, average, cumulative_data))
  } else{
    out <- paste0("<h4> </h4><br /><h4>Group A</h4><br />",
                  enrollment_by_site_last_days_var_disc_i(df_a, days, average, cumulative_data) %>% add_footnote(footnotes, notation="number", escape = FALSE),
                  "<h4>Group B</h4><br />",
                  enrollment_by_site_last_days_var_disc_i(df_b, days, average, cumulative_data) %>% add_footnote(footnotes, notation="number", escape = FALSE))
  }
...
#'
#' @description 
#' This is the closed version of the enrollment_by_site_last_days_var_disc_ii function; see its documentation
#' for details.
#'
#' @param analytic analytic data set that must include constructs screened, eligible, refused, consented, 
#' not_consented, not_randomized, consented_and_randomized, enrolled, site_certification_date, 
...
#'
#' @examples
#' closed_enrollment_by_site_last_days_var_disc_ii("Replace with Analytic Tibble")
#' closed_enrollment_by_site_last_days_var_disc_ii("Replace with Analytic Tibble", days = 100)
#' closed_enrollment_by_site_last_days_var_disc_ii("Replace with Analytic Tibble", include_exclusive_safety_set = FALSE, footnotes = TRUE)
#' closed_enrollment_by_site_last_days_var_disc_ii("Replace with Analytic Tibble", include_exclusive_safety_set = TRUE)
#' 
closed_enrollment_by_site_last_days_var_disc_ii <- function(analytic, discontinued="discontinued", 
...
#' enrolled, not_consented, site_certification_date, facilitycode, treatment_arm
...
#' @param pre_screened name of the construct for pre-screened status defaults to NULL
#' @param pre_screened_eligible name of the construct for pre-screened eligible status defaults to NULL
...
#' @examples
#' \dontrun{
#' closed_generic_characteristics(analytic, constructs=c("age_cat", "age"), names_vec=c("Age", NA), mean_sd=c("age"))
#' }
closed_generic_characteristics <- function(analytic, constructs = c(), names_vec = c(), 
                                           ...
                                           mean_sd = c()){
  confirm_stability_of_related_visual('generic_characteristics', 'a51f72e169642b9bc1d7d893052b4c65')
  ...
  for (i in seq(length(constructs))) {
    ...
    if (is_mean_sd) {
      ...
      if (is.na(name_str) || name_str == "") {
        ...
        new <- 1; names(new) <- paste0(name_str, ' (Group A=',a_total,', Group B=',b_total,', n=', total, ')')
        index_vec <- c(index_vec, new); has_border <- c(has_border, FALSE)
      }
      
      if (is.null(out)) out <- inner else out <- bind_rows(out, inner)
      
    } else {
      inner <- inner_analytic %>% mutate(temp = !!sym(construct)) %>% mutate(temp = replace_na(as.character(temp), "Missing"))
      if(!is.na(sub_construct)) inner <- inner %>% mutate(sub_temp = !!sym(sub_construct)) %>% mutate(sub_temp = replace_na(as.character(sub_temp), "Missing"))
      
      inner_split <- splits[i]
      if(!is.na(inner_split)) inner <- inner %>% separate_rows(temp,sep = inner_split)
      ...
      numeric_sort_list <- non_bottom_temps[is_numeric] %>% as.numeric() %>% sort() %>% as.character()
      ...
      if(!is.na(sub_construct)){
        ...
        if("Missing" %in% sub_cats) sub_cats <- c(sub_cats[sub_cats != "Missing"],"Missing")
        
        row_count <- ifelse(is.null(out),0,nrow(out))
        new_row_count <- 0
        for(sub_cat in sub_cats){
          category_df <- inner %>% filter(sub_temp==sub_cat) %>% select(-sub_temp) %>% group_by(temp, treatment_arm) %>% count(temp)
          category_df_all <- inner %>% filter(sub_temp==sub_cat) %>% select(-sub_temp) %>% group_by(temp) %>% count(temp) %>% 
            mutate(Total = format_count_percent(n, sum(category_df$n))) %>% mutate(header = name_str) %>%
            mutate(temp = factor(temp, levels = custom_levels)) %>% arrange(temp) %>% mutate(temp = as.character(temp))
          ...
          tot_df <- tibble(temp=sub_cat, header=name_str, "Group A"=format_count_percent(category_tot_a, total),
                           "Group B"=format_count_percent(category_tot_b, total), Total=format_count_percent(category_tot, total))
          
          category_df <- category_df %>% mutate(percentage = ifelse(treatment_arm == 'Group A', format_count_percent(n,  category_tot_a),
                                                                    ifelse(treatment_arm == 'Group B', format_count_percent(n,  category_tot_b), NA))) %>% 
            select(-n) %>% mutate(header = name_str) %>% pivot_wider(names_from = treatment_arm, values_from = percentage, values_fill = "0 (0%)")
          
          category_df <- full_join(category_df_all %>% select(-n), category_df, by = c("temp", "header"))
          if(!"Group A" %in% colnames(category_df)) category_df <- category_df %>% mutate("Group A" = "0 (0%)")
          if(!"Group B" %in% colnames(category_df)) category_df <- category_df %>% mutate("Group B" = "0 (0%)")
          
          category_df <- category_df %>% select(temp, header, `Group A`, `Group B`, Total)
          if (titlecase) category_df <- category_df %>% mutate(temp = str_to_title(temp))
          if (is.null(out)) out <- bind_rows(tot_df, category_df) else out <- bind_rows(out, tot_df, category_df)
        ...
        if (is.na(name_str) || name_str == "") {
          if (length(index_vec) > 0) index_vec[length(index_vec)] <- index_vec[length(index_vec)] + new_row_count
          else { new <- new_row_count; names(new) <- " "; index_vec <- c(index_vec, new); has_border <- c(has_border, TRUE) }
        } else {
          new <- new_row_count; names(new) <- paste0(name_str, ' (Group A=',a_total,', Group B=',b_total,', n=', total, ')')
          index_vec <- c(index_vec, new); has_border <- c(has_border, TRUE)
        }
        
      } else{
        inner_some <- inner %>% group_by(temp, treatment_arm) %>% count(temp) %>% mutate(percentage = format_count_percent(n, total)) %>% 
          select(-n) %>% mutate(header = name_str) %>% pivot_wider(names_from = treatment_arm, values_from = percentage, values_fill = "0 (0%)")%>%
          select(any_of(c("temp", "header", "Group A", "Group B")))
        
        inner_all <- inner %>% group_by(temp) %>% count(temp) %>% mutate(Total = format_count_percent(n, total)) %>% 
          select(-n) %>% mutate(header = name_str) %>% mutate(temp = factor(temp, levels = custom_levels)) %>% 
          arrange(temp) %>% mutate(temp = as.character(temp))
        
        inner <- full_join(inner_all, inner_some, by = c("temp", "header"))
        
        if(!"Group A" %in% colnames(inner)) inner <- inner %>% mutate(`Group A` = "0 (0%)")
        if(!"Group B" %in% colnames(inner)) inner <- inner %>% mutate(`Group B` = "0 (0%)")
        inner <- inner %>% select(temp, header, `Group A`, `Group B`, Total)
        if (titlecase) inner <- inner %>% mutate(temp = str_to_title(temp))
        
        if (is.na(name_str) || name_str == "") {
          if (length(index_vec) > 0) index_vec[length(index_vec)] <- index_vec[length(index_vec)] + nrow(inner)
          else { new <- nrow(inner); names(new) <- " "; index_vec <- c(index_vec, new); has_border <- c(has_border, TRUE) }
        } else {
          new <- nrow(inner); names(new) <- paste0(name_str, ' (Group A=',a_total,', Group B=',b_total,', n=', total, ')')
          index_vec <- c(index_vec, new); has_border <- c(has_border, TRUE)
        }
...
#' eligible, refused, consented, enrolled, not_consented, discontinued_pre_randomization, site_certification_date, 
...
closed_enrollment_status_by_site <- function(analytic){
  ...
  analytic <- if_needed_generate_example_data(
    ...
    example_constructs = c('screened', 'eligible', 'refused', 'consented', 'enrolled', 
                           'not_consented', 'discontinued_pre_randomization', 'site_certification_date', 
...
#'
#' @param analytic This is the analytic dataset that must include enrolled and treatment_arm
...
#'
#' @description 
#' This is the closed version of the enrollment_status_by_site_consent_pre_screening_i function; see its documentation
#' for details.
#'
#' @param analytic analytic data set that must include constructs screened, eligible, refused, consented, 
#' not_consented, not_randomized, consented_and_randomized, enrolled, site_certification_date, 
...
#'
#' @description 
#' This is the closed version of the enrollment_status_by_site_consent_pre_screening_ii function; see its documentation
#' for details.
#'
#' @param analytic analytic data set that must include constructs screened, eligible, refused, consented, 
#' not_consented, not_randomized, consented_and_randomized, enrolled, site_certification_date, 
