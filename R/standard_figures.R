...
dsmb_consort_diagram <- function(analytic, not_enrolled_other=NULL, completed_str="Completed 12-month visit"){
  ...
  img_tag <- sprintf('<img src="data:image/png;base64,%s" alt="Consort Diagram" style="width: 95vw;">', image_data)
...
cumulative_percentage_ankle_injuries <- function(analytic){
  ...
  image_data <- base64enc::base64encode(temp_png_path)
  img_tag <- sprintf('<img src="data:image/png;base64,%s" alt="Cumulative Percentage Enrollment for Ankle injury" style="width: 95vw;">', image_data)
...
cumulative_percentage_plateau_injuries <- function(analytic){
  ...
  ggsave(temp_png_path, plot = g, width = 2500, height = 1000, units = 'px')
  image_data <- base64enc::base64encode(temp_png_path)
  img_tag <- sprintf('<img src="data:image/png;base64,%s" alt="Cumulative Percentage Enrollment for Plateau injury" style="width: 95vw;">', image_data)
  file.remove(temp_png_path)
...
enrollment_by_injury_and_site <- function(analytic){
  ...
  ggsave(temp_png_path, plot = g, width = 2500, height = 1000, units = 'px')
  image_data <- base64enc::base64encode(temp_png_path)
  img_tag <- sprintf('<img src="data:image/png;base64,%s" alt="Enrollment by each injury type and site" style="width: 95vw;">', image_data)
  file.remove(temp_png_path)
